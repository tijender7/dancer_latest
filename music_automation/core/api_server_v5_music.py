...
CONFIG_FILE = "config/config_music.json"
try:
    script_dir = Path(__file__).resolve().parent.parent  # Go up to music_automation root
...
BASE_WORKFLOW_VIDEO_PATH = (script_dir.parent / "base_workflows" / "api_wanvideo_without_faceswap.json").resolve()
...
# Different output prefix nodes for different workflows
IMAGE_OUTPUT_PREFIX_NODE_TITLE = "API_Base_Output_Prefix"  # For image workflow
VIDEO_OUTPUT_PREFIX_NODE_TITLE = "API_Output_Prefix"       # For video workflow
...
logger.info(f"  API Server Port: 8006 (Music Pipeline)")
...
class MusicGenerationRequest(BaseModel):
    ...
    segment_id: int | None = Field(None, description="Segment number from music timeline (optional for compatibility)")
...
def prepare_and_submit_workflow(
    ...
    try:
        # --- Find Nodes ---
        segment_info = f"Segment {request.segment_id}" if request.segment_id else "No segment"
        logger.info(f"[{client_id}] Finding nodes for {wf_name_log} ({segment_info})...")
        ...
        # Use different output prefix node based on workflow type
        output_prefix_title = IMAGE_OUTPUT_PREFIX_NODE_TITLE if workflow_type == "Image" else VIDEO_OUTPUT_PREFIX_NODE_TITLE
        logger.info(f"[{client_id}] Looking for output prefix node: '{output_prefix_title}'")
        output_prefix_node_id = find_node_id_by_title(wf, output_prefix_title, wf_name_log)
        ...
        if not output_prefix_node_id: 
            raise ValueError(f"Could not find Output Prefix node '{output_prefix_title}'.")
        ...
        if output_prefix_node_id:
            ...
            if prefix_node_class == "FileNamePrefix":
                ...
                filename_prefix = f"{request.filename_prefix_text}{segment_suffix}"
            ...
            logger.error(f"[{client_id}] Output Prefix node '{output_prefix_title}' ID not found, skipping output path injection.")
...
async def generate_image(request: MusicGenerationRequest):
    ...
    segment_info = f"segment {request.segment_id}" if request.segment_id else "no segment"
    logger.info(f"[{client_id}] Image generation request for {segment_info}")
    ...
    try:
        ...
        if results["status"] == "submitted":
            response_data = {
                "status": "submitted",
                "message": f"Image generation started for {segment_info}",
...
async def generate_video(request: MusicGenerationRequest):
    ...
    client_id = str(uuid.uuid4())
    segment_info = f"segment {request.segment_id}" if request.segment_id else "no segment"
    logger.info(f"[{client_id}] Video generation request for {segment_info}")
    ...
    try:
        ...
        if results["status"] == "submitted":
            return {
                "status": "submitted",
                "message": f"Video generation started for {segment_info}",
                "prompt_id": results["prompt_id"],
                "client_id": client_id,
                "segment_id": request.segment_id
            }
        ...
        logger.error(f"[{client_id}] Video generation failed for {segment_info}: {str(e)}")
    ...
    return {
        ...
        "config": {
            ...
            "node_titles": {
                ...
                "image_output_prefix": IMAGE_OUTPUT_PREFIX_NODE_TITLE,
                "video_output_prefix": VIDEO_OUTPUT_PREFIX_NODE_TITLE,
