# main_automation_v2.py (Enhanced Logging)
import os
import json
import random
import requests
...
OLLAMA_MAX_RETRIES = 3 # Corrected constant name
OLLAMA_RETRY_DELAY = 3
OLLAMA_TIMEOUT = 180
REQUEST_TIMEOUT = 60 # Increased timeout for API server calls slightly
...
# Use a simpler format for console for better readability during run
console_formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
console_handler.setFormatter(console_formatter)
logger = logging.getLogger()
logger.setLevel(logging.INFO) # Set to DEBUG for extremely verbose output
...
# (Keep the load_config function as is - it already logs well)
def load_config(config_path="config2.json"): # Ensure correct filename used
    ...
    try:
        ...
        if 'api_server_url' not in config: logger.critical("CRITICAL: 'api_server_url' missing in config.json"); sys.exit(1)
...
# (Keep the create_run_folder function as is)
...
# (Keep the generate_prompts_ollama function as is, using OLLAMA_MAX_RETRIES)
def generate_prompts_ollama(model, num_prompts, ollama_api_url):
    ...
    backgrounds = [ "neon-drenched cyberpunk street", "ancient mossy temple ruins", "zero-gravity space station lounge", "steampunk airship deck", "surreal Dali-esque desert landscape", "art deco speakeasy bar", "bioluminescent underwater cavern", "vibrant Dia de Muertos festival", "minimalist Japanese zen garden", "post-apocalyptic overgrown city" ]
                    ...
                    if start_index != -1 and end_index != -1 and start_index < end_index:
                        ...
                        if "prompts" in parsed and isinstance(parsed["prompts"], list) and parsed["prompts"]:
                            ...
                            if prompt_text:
                                ...
                                generated_prompt_list.append({"index": i + 1, "background": background, "generated_prompt": prompt_text}); ollama_success = True; break
...
# --- Function to Call API Server (Enhanced Logging) ---
def trigger_generation(api_url: str, endpoint: str, prompt: str, face_filename: str | None, output_subfolder: str | None):
    """Sends a request to the specified API server endpoint with detailed logging."""
    full_url = f"{api_url.rstrip('/')}/{endpoint.lstrip('/')}"
    payload = {"prompt": prompt, "face": face_filename or "", "output_subfolder": output_subfolder}

    log_prefix = f"API Call -> {endpoint}"

    # Log the details being sent *before* the loop starts
    logger.info(f"  ➡️ {log_prefix}: Preparing request...")
    logger.info(f"     URL: {full_url}")
    logger.info(f"     Prompt (start): '{prompt[:70]}...'") # Log beginning of prompt
    logger.info(f"     Face Filename: '{face_filename or 'None'}'")
    logger.info(f"     Output Subfolder: '{output_subfolder or 'None (Workflow Default)'}'")
    logger.debug(f"    Payload Sent: {json.dumps(payload, indent=2)}") # DEBUG level for full payload
    ...
    for attempt in range(1, MAX_API_RETRIES + 1):
        ...
        try:
            ...
            # Log key details from the API server's response
            api_status = response_data.get('status', 'N/A')
            api_error = response_data.get('error', None)
            comfy_prompt_id = response_data.get('prompt_id', 'N/A') # Get ComfyUI's internal ID
            ...
            # logger.debug(f"    Full API Response: {json.dumps(response_data, indent=2)}") # Optional: Log full response at DEBUG
            return True # Success
            ...
            if e.response is not None:
             ...
             logger.debug(f"     Raw Response: {response.text[:500]}...") # Log raw response if JSON fails
...
# --- Main Execution Logic (Enhanced Logging) ---
if __name__ == "__main__":
    logger.info("=" * 50); logger.info(f"Starting Automation v2 Run: {datetime.now()}"); logger.info("=" * 50)
    config = load_config("config2.json"); API_SERVER_URL = config['api_server_url'] # Load correct config
    ...
    for item in image_progress_bar:
        ...
        # Create subfolder name - used for both local structure and API param
        run_subfolder_name = f"{idx:03d}_{''.join(c if c.isalnum() or c in ['-','_'] else '_' for c in background)[:30]}_{datetime.now().strftime('%H%M%S%f')}" # Added microseconds for more uniqueness
        # We don't create the folder here anymore, just pass the name
        # current_run_output_dir = script_output_root / run_subfolder_name
        # current_run_output_dir.mkdir(exist_ok=True)

        # Store details needed for the video stage
        prompt_details_for_video[idx] = {"prompt": prompt, "face_filename": face_filename_only, "run_subfolder_name": run_subfolder_name}

        # Define the subfolder structure to request from ComfyUI
        ...
        # time.sleep(0.5) # Optional very short delay
    ...
    # ====================================================
    logger.info(f"\n--- STAGE 2: Submitting Video Generation for {len(valid_prompts)} Prompts ---")
    video_progress_bar = tqdm(valid_prompts, desc="Submitting Videos")
    for item in video_progress_bar:
        idx = item["index"]
        details = prompt_details_for_video.get(idx)
        if not details: logger.error(f"Could not find details for prompt index {idx}. Skipping video generation."); continue

        prompt = details["prompt"]
        face_filename_only = details["face_filename"]
        run_subfolder_name = details["run_subfolder_name"] # Use the same unique name from image stage

        # Define the subfolder structure for ComfyUI's video output
        video_output_subfolder = f"{run_subfolder_name}/videos"

        video_progress_bar.set_description(f"Video Req {idx}/{len(valid_prompts)}")
        logger.info(f"\n🎬 Preparing Video Request [{idx}/{len(valid_prompts)}] | Request Subfolder: '{video_output_subfolder}'")
    ...
    logger.info(f"--- STAGE 2: All Video Generation Requests Submitted ---")
    logger.info(f"--- NOTE: ComfyUI is likely still processing these in the background. ---")


    logger.info("\n" + "=" * 50)
    logger.info(f"🎉 Automation v2 Run Script Finished! {datetime.now()}")
