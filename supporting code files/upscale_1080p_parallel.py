...
#  CONFIGURATION
# ========================================================================
COMFYUI_OUTPUT_DIR_BASE = Path(r"H:/dancers_content")  # Base directory
VIDEO_PARENT_SUBFOLDER = "all_videos"
COMPILED_SUBFOLDER = "compiled"
UPSCALE_SUBFOLDER = "1080p_upscaled"
...
TOPAZ_TIMEOUT = 3600  # seconds

TARGET_BITRATE_KBPS = "6000k"
MAX_BITRATE_KBPS = "10000k"
AUDIO_BITRATE_KBPS = "128k"
...
TOPAZ_FILTER_COMPLEX = (
    ...
    "scale=w=min(iw*1\\,1920):h=min(ih*1\\,1080):force_original_aspect_ratio=decrease:flags=lanczos,split[s1][s2];"
    "[s1]format=pix_fmts=yuv420p[scaled];"
    "[s2]pad=w=1920:h=1080:x=-1:y=-1:color=black[padded];"
    "[scaled][padded]overlay=shortest=1"
)
...
log_file = log_directory / f"upscale_1080p_parallel_run_{datetime.now():%Y%m%d_%H%M%S}.log"
...
def upscale_video_topaz(task_id, input_video_path: Path, output_video_path: Path):
    label = f"1080p Upscale Task {task_id}"
    ...
    try:
        ...
        success = (result.returncode == 0 and output_video_path.exists() and output_video_path.stat().st_size > 5120)
        ...
        if success:
            ...
            elif output_video_path.stat().st_size <= 5120:
                logger.error(f"       Output file invalid or too small: {output_video_path} ({output_video_path.stat().st_size} bytes)")
            if output_video_path.exists() and output_video_path.stat().st_size <= 5120:
                try:
...
# ========================================================================
#  Main Script Logic - Upscale from compiled folder
# ========================================================================
if __name__ == "__main__":
    logger.info("=" * 50); logger.info(f"Starting Parallel 1080p Upscaling Script: {datetime.now()}"); logger.info("=" * 50)
    ...
    logger.info(f"1080p upscaled videos will be saved to: {upscaled_output_parent_dir}")
    ...
    for i, video_path in enumerate(video_files):
        ...
        if upscaled_output_path.exists() and upscaled_output_path.stat().st_size > 5120:
    ...
    logger.info(f"\n--- Starting Topaz 1080p Video Upscaling from compiled ({len(tasks)} tasks, {MAX_CONCURRENT_UPSCALES} parallel) ---")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_UPSCALES) as executor:
        ...
        for future in tqdm(concurrent.futures.as_completed(future_to_task), total=len(tasks), desc="Upscaling 1080p Videos"):
    ...
    logger.info(f"\n--- Finished 1080p Upscaling ({successful_upscales} successful, {failed_upscales} failed) ---")
    logger.info(f"Upscaled 1080p videos saved in: {upscaled_output_parent_dir}")
    logger.info("=" * 50)
