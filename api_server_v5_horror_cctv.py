# api_server_v5_horror_cctv.py (Handles Horror CCTV LoRA Workflow)
import os
import json
import requests
import uuid
...
logger.info("Initializing Horror CCTV API Server v5...")

# --- Configuration Loading ---
CONFIG_FILE = "config_horror_cctv.json" # Uses horror CCTV config
try:
    ...
    logger.info(f"Horror CCTV config loaded successfully from '{CONFIG_FILE}'.")
...
logger.info(f"Horror CCTV Image Workflow: {BASE_WORKFLOW_IMAGE_PATH.name}")
...
logger.info(f"Horror CCTV LoRA Trigger Word: {HORROR_CCTV_TRIGGER}")
...
try:
    ...
    logger.info("Horror CCTV base workflows loaded successfully.")
...
# --- FastAPI App ---
app = FastAPI(title="Horror CCTV ComfyUI Generation API v5")
...
class GenerationRequest(BaseModel):
    ...
    filename_prefix_text: str = Field(..., description="Text to include in the filename prefix (e.g. '001_horror_cctv')")
...
# --- Helper Function: Inject Horror CCTV Trigger ---
def inject_horror_cctv_trigger(prompt: str) -> str:
    """Ensures the Horror CCTV trigger word is present in the prompt."""
    if HORROR_CCTV_TRIGGER not in prompt:
        # Add trigger word at the beginning
        return f"{HORROR_CCTV_TRIGGER},{prompt}"
    return prompt

# --- Core Function: Generate Workflow (Enhanced for Horror CCTV) ---
def prepare_workflow(
    ...
    """
    Prepares a workflow JSON with the given parameters for Horror CCTV generation.
    
    Args:
        prompt: Text prompt for generation
        face: Filename of face image (not used in horror CCTV - no face swap)
    ...
    logger.info(f"Preparing {workflow_name} workflow for Horror CCTV generation...")
    
    # Inject Horror CCTV trigger word into prompt
    enhanced_prompt = inject_horror_cctv_trigger(prompt)
    if enhanced_prompt != prompt:
        logger.info(f"Enhanced prompt with trigger word: {HORROR_CCTV_TRIGGER}")
    ...
    # 1. Set prompt (with Horror CCTV trigger)
    ...
    # Note: Face swap is skipped entirely for horror CCTV automation
    
    logger.info(f"Horror CCTV workflow prepared successfully: {workflow_name}")
    return workflow
...
async def root():
    ...
    return {
        "message": "Horror CCTV API Server v5 is running",
...
async def generate_image(request: GenerationRequest):
    """
    Generate images using the Horror CCTV workflow.
    
    The prompt will automatically have the Horror CCTV trigger word added if not present.
    Face swap is not supported in this automation.
    """
    logger.info(f"Received horror CCTV image generation request")
    logger.info(f"  Prompt: '{request.prompt[:100]}...'")
    logger.info(f"  Output Subfolder: '{request.output_subfolder}'")
    logger.info(f"  Filename Prefix: '{request.filename_prefix_text}'")
    logger.info(f"  Face: '{request.face}' (ignored - no face swap in horror CCTV)")
    ...
    try:
        ...
        return {
            "status": "submitted",
            "prompt_id": prompt_id,
            "trigger_word_used": HORROR_CCTV_TRIGGER,
            "enhanced_prompt": inject_horror_cctv_trigger(request.prompt),
            "message": "Horror CCTV image generation submitted to ComfyUI"
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error in horror CCTV image generation: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Unexpected error: {e}")

@app.post("/generate_video")
async def generate_video(request: GenerationRequest):
    """
    Generate videos using the existing video workflow with Horror CCTV trigger word.
    
    The prompt will automatically have the Horror CCTV trigger word added if not present.
    Face swap is not supported in this automation.
    """
    logger.info(f"Received horror CCTV video generation request")
    logger.info(f"  Prompt: '{request.prompt[:100]}...'")
    logger.info(f"  Output Subfolder: '{request.output_subfolder}'")
    logger.info(f"  Filename Prefix: '{request.filename_prefix_text}'")
    ...
    try:
        ...
        return {
            ...
            "message": "Horror CCTV video generation submitted to ComfyUI"
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Unexpected error in horror CCTV video generation: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Unexpected error: {e}")

# --- Server Startup ---
if __name__ == "__main__":
    logger.info("Starting Horror CCTV API Server v5...")
    logger.info(f"Trigger word: {HORROR_CCTV_TRIGGER}")
    logger.info(f"Image workflow: {BASE_WORKFLOW_IMAGE_PATH.name}")
    logger.info(f"Video workflow: {BASE_WORKFLOW_VIDEO_PATH.name}")
    logger.info("Server will run on http://0.0.0.0:8002")
