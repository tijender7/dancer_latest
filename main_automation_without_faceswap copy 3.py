...
MODIFIED: Enhanced prompting module integrated while keeping everything else intact.
"""
...
print("DEBUG: Script execution started (v6 with Enhanced Prompting).")
...
log_file = log_directory / f"automation_v6_enhanced_prompting_run_{datetime.now():%Y%m%d_%H%M%S}.log"
...
logger.info("Starting Automation v6 Run with Enhanced Prompting")
...
# --- START: ENHANCED PROMPT GENERATION MODULE ---

# Dictionary to hold modular prompt components for high variety
PROMPT_ELEMENTS = {
    "locations": [
        "a vibrant Bollywood dance festival in Mumbai with colorful decorations",
        "a luxurious rooftop party overlooking the Gateway of India in Mumbai",
        "a traditional Indian wedding celebration with marigold flowers and fairy lights",
        "a high-energy Punjabi bhangra competition in a decorated mandap",
        "a glamorous Indian fashion show with runway lights and ethnic décor",
        "a festive Holi celebration with colorful powder and traditional music",
        "an exclusive pool party at a luxury resort in Goa with palm trees",
        "a vibrant Navratri garba dance festival with ornate decorations",
        "a modern Indian nightclub with neon lights and traditional fusion music",
        "a beachside party in Kerala during sunset with coconut palms",
        "a royal palace courtyard in Rajasthan converted into a dance venue",
        "a contemporary Indian cultural festival with modern and traditional elements",
    ],
    "attires": [
        ...
        "a contemporary lehenga with a strapless, push-up choli and a very short, flowy skirt that shows her legs and curves",
        "a traditional Rajasthani outfit modified with a very low-cut, sleeveless choli and a short ghagra with a high slit",
        "a bold, modern saree style with a tiny halter-neck blouse and the saree draped to show maximum skin and curves",
        "a festive crop top with mirror work paired with ultra-short, tight-fitting churidar pants that hug her curves",
        "a revealing Punjabi suit with a very short, low-cut kameez and tight salwar that shows her figure",
    ],
    "dance_actions": [
        "performing energetic bhangra moves with dramatic shoulder shaking and hip movements",
        "executing graceful classical Indian dance poses that emphasize her curves and chest movements",
        "doing seductive Bollywood-style hip sways and chest pops while moving toward the camera",
        "performing traditional garba spins modified with modern, sensual body movements",
        ...
        "performing rhythmic thumri-inspired moves with graceful arm movements and dramatic body rolls",
    ],
    "camera_shots": [
        "a dynamic low-angle shot emphasizing her powerful dance moves and dominant presence",
        "an intimate close-up focusing on her expressive Indian features and bouncing chest",
        "a cinematic medium shot that captures her full dance movements against the Indian backdrop",
        "a dramatic side-angle shot highlighting the curves of her silhouette and hip movements",
        "a slow-motion full-body shot that emphasizes every bounce and sway of her figure",
        "a follow-shot from behind focusing on her hip movements and the flow of her outfit",
    ],
    "lighting": [
        "bathed in warm, golden Indian sunset light creating a ethereal glow around her",
        "dramatically lit by colorful festival lights and diyas creating vibrant shadows",
        "illuminated by soft, romantic moonlight mixed with traditional oil lamps",
        "backlit by bright Bollywood-style stage lights creating a star-like silhouette",
        "under the warm glow of traditional Indian lanterns and fairy lights",
        "in bright, high-key sunlight typical of Indian celebrations and festivals",
    ]
}

def generate_prompts_ollama(model, num_prompts, ollama_api_url):
    """
    Generates prompts using a modular, high-variety system.
    """
    logger.info(f"🚀 Generating {num_prompts} prompts via Ollama with HIGH-VARIETY module (Model: {model})...")
    generated_prompt_list = []

    for i in range(num_prompts):
        # Dynamically construct a unique scene from random elements
        location = random.choice(PROMPT_ELEMENTS["locations"])
        attire = random.choice(PROMPT_ELEMENTS["attires"])
        ...
        dynamic_scene_description = (
            f"The main subject is a gorgeous, stunning Indian woman with extremely voluptuous curves, very large natural breasts, "
            f"and an incredibly sexy, confident presence. She has beautiful Indian features, long dark hair, and radiant skin. "
            f"She is wearing {attire} that shows maximum skin while remaining tasteful. "
            f"She is at {location}. "
            f"The lighting is {lighting}. "
            f"She is {dance_action}. "
            f"The camera is capturing this with {camera_shot}. "
            f"The atmosphere is high-energy, sensual, and culturally rich. Her outfit reveals her curves, cleavage, midriff, "
            f"and hips in an artistic, non-nude but maximally revealing way. She is the absolute focus with incredible sex appeal."
        )
        ...
        formatted_prompt = (
            ...
            The final prompt must be a masterpiece of visual description focusing on a stunning Indian woman.
            Strictly adhere to all details provided. Do not add commentary or extra text.
            The main subject must be a beautiful Indian woman with voluptuous curves, large breasts, and revealing traditional/modern Indian clothing.
            The dance must be energetic Indian dance styles with chest, hip, and body movements, no spinning.
            Emphasize her Indian heritage, cultural elements, and maximum skin showing in a tasteful, artistic way.
        ...
        logger.info(f"\n🧠 Requesting Prompt [{i+1}/{num_prompts}] | Theme: {location}")
        logger.debug(f"   Generated Scene Details for Ollama:\n{dynamic_scene_description}")
                        ...
                        if "prompts" in parsed and isinstance(parsed["prompts"], list) and parsed["prompts"]:
                            ...
                            if prompt_text:
                                ...
                                generated_prompt_list.append({
                                    "index": i + 1,
                                    "background": location,  # Use the specific location as the theme
    ...
    logger.info(f"✅ Finished generating {successful_count}/{num_prompts} high-variety prompts.")
...
if __name__ == "__main__":
    ...
    logger.info("=" * 50)
    logger.info(f"Starting Automation v6 (Enhanced Prompts) Run: {datetime.now()}")
    logger.info("=" * 50)
