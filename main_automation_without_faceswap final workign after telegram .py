#!/usr/bin/env python
"""Sequentially run the dancer automation pipeline with parallel approval (Web UI or Telegram).

This script starts the intermediate API server in the background, generates images,
and then offers you a choice between Web-based approval or Telegram-based approval.
After approval, it proceeds to generate videos and clean up.
"""
...
if __name__ == "__main__":
    ...
    # ====================================================
    # STAGE 1.7: Image Approval (Web UI or Telegram)
    # ====================================================
    ...
    if not run_details_for_approval:
        ...
        logger.error(f"ComfyUI Output Base Dir ({COMFYUI_OUTPUT_DIR_BASE}) not found. Cannot start approval. Skipping approval.")
    else:
        # Ask user which method to use
        def ask_approval_method():
            print("Choose approval method:")
            print("1. Web UI")
            print("2. Telegram")
            while True:
        ...
        approval_method = ask_approval_method()

        if approval_method == "1":
            # ─── Web UI Approval ──────────────────────────────────────────
            logger.info("[INFO] Using Web UI for image approval...")
            ...
            try:
                ...
                logger.error(f"Failed to create approved images folder: {e}. Skipping web approval.", exc_info=True)
                run_details_for_approval = []
            ...
            if run_details_for_approval:
                ...
                logger.info("Starting Web UI approval server thread...")
                ...
                shutdown_event.wait()  # Block until approval submission

                logger.info("Web UI approval done, processing approvals...")
                ...
                if approval_file_path.is_file():
                    try:
                        with open(approval_file_path, 'r', encoding='utf-8') as f:
                            ...
                            logger.info(f"Loaded {len(approved_image_details)} approved image details.")
                    except Exception as e:
                ...
                if approved_image_details:
                    logger.info("Copying approved images (Web UI)...")
                    copied_count = 0
                    ...
                    logger.info("No images were approved via Web UI.")

        else:
            # ─── Telegram Approval ────────────────────────────────────────
            logger.info("[INFO] Sending images to Telegram for approval...")
            if not TELEGRAM_APPROVALS_DIR.exists():
                TELEGRAM_APPROVALS_DIR.mkdir(parents=True, exist_ok=True)
            # Clear any existing approvals file
            if TELEGRAM_APPROVALS_JSON.exists():
                TELEGRAM_APPROVALS_JSON.unlink()

            # Build command: python send_telegram_image_approvals.py <img1> <img2> ...
            cmd = [
                sys.executable,
            ...
            try:
                ...
                logger.error(f"Failed to run Telegram sender script: {e}")
                sys.exit(1)

            # Poll telegram_approvals.json until all statuses are set
            ...
            while True:
                ...
                if TELEGRAM_APPROVALS_JSON.exists():
                    with open(TELEGRAM_APPROVALS_JSON, "r", encoding="utf-8") as f:
                        data = json.load(f)
                    pending = [img for img, val in data.items() if val.get("status") is None]
                    if not pending:
                        break
            # Build approved_image_details list from data
            with open(TELEGRAM_APPROVALS_JSON, "r", encoding="utf-8") as f:
                data = json.load(f)
            approved_image_paths = [img for img, val in data.items() if val.get("status") == "approve"]
            logger.info(f"Telegram: Approved {len(approved_image_paths)} images, Rejected {len(data)-len(approved_image_paths)}.")

            # Map approved_image_paths back to run_details_for_approval to build approved_image_details
            approved_image_details = []
            for img_path_str in approved_image_paths:
                img_path = Path(img_path_str)
                # Find in which run_details item this image belonged
                found_detail = None
                for item in run_details_for_approval:
                    if img_path in item['generated_image_paths']:
                        found_detail = item
                        break
                if not found_detail:
                    logger.warning(f"Telegram: Could not find run_details entry for {img_path}. Skipping.")
                    continue

                orig_index = found_detail['index']
                prompt = found_detail['prompt']
                face_filename = found_detail['face_filename']
                image_prefix = found_detail['image_prefix']
                batch_index = found_detail['generated_image_paths'].index(img_path)
                ...
                approved_image_details.append({
                    ...
                    "base_image_prefix": image_prefix
                })

            if approved_image_details:
                logger.info("Copying approved images (Telegram)...")
                approved_images_folder_path = main_run_folder_path / APPROVED_IMAGES_SUBFOLDER
                try:
                    approved_images_folder_path.mkdir(exist_ok=True)
                except Exception as e:
                ...
                logger.info("No images were approved via Telegram.")
